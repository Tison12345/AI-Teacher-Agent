...
def start_session(body: StartSessionRequest):
    ...
    try:
        # Step 1: auto-detect unknown tech stack from project description
        state = stack_identifier_node.run(state)

        # Step 2: generate prerequisite concepts
        state = prerequisite_node.run(state)

        # Step 3: generate quiz for concept 0
