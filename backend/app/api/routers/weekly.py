...
class EvaluationBreakdown(BaseModel):
    ...
    reflection_points:     Optional[int] = None
...
@router.post("/{session_id}/skip", response_model=CheckCompletionResponse)
def skip_completion(session_id: str):
    """[TEST] Mark this week as complete without a GitHub check."""
    state = store.get(session_id)
    if not state:
        raise HTTPException(status_code=404, detail="Session not found")
    if not state.get("weekly_tasks"):
    ...
    state = {
        **state,
        ...
        "completion_reason": "Skipped for testing — no GitHub check performed.",
    }
    store.set(session_id, state)
...
@router.post("/{session_id}/skip-to-dashboard", response_model=EvaluationResponse)
def skip_to_dashboard(session_id: str):
    """[TEST] Skip completion check + quiz and jump straight to the dashboard evaluation."""
    state = store.get(session_id)
    if not state:
        raise HTTPException(status_code=404, detail="Session not found")
    if not state.get("weekly_tasks"):
    ...
    try:
        ...
        state = {**state, "completion_status": True, "completion_reason": "Skipped for testing."}

        # Generate a quiz so the evaluator has data to work with
        state = task_quiz_node.run(state)

        # Grade with empty answers (score = 0, passes because PASS_THRESHOLD = 0)
        n_questions = len(state["task_quiz_results"][-1]["questions"])
        state = task_quiz_node.grade_quiz(state, [""] * n_questions)

        # Run evaluator
        state = evaluator_node.run(state)

    except Exception as e:
        logger.error(f"skip_to_dashboard error: {e}")
        raise HTTPException(status_code=500, detail=str(e))
