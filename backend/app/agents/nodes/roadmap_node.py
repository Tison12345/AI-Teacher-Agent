...
class RoadmapNode:
    ...
    ROADMAP_PROMPT = """Create a week-by-week project roadmap and return JSON.
...
STUDENT GAPS (use only to adjust pacing and add teaching moments — do NOT make gaps the weekly theme):
{gaps_summary}

FOCUS AREAS (weave naturally into weeks — they should NOT dominate):
{focus_areas}

QUIZ STRUGGLES (concepts to reinforce as the project progresses):
{quiz_summary}
...
RULES
1. The roadmap is about BUILDING THE PROJECT — each week advances the actual product, not just student weaknesses.
2. Follow natural software development phases: Setup → Core features → Advanced features → Polish/Deploy.
3. Week 1 = project setup and foundation (folder structure, basic boilerplate, first working screen or endpoint).
4. Each week after that builds a real, visible feature that makes the project more complete.
5. Final week = polish, testing, README, and deployment-ready code.
6. Gaps and focus areas should quietly shape HOW you teach within a week, not WHAT the week is about.
7. Difficulty arc: easy (week 1) → medium → hard → medium (polish week).
8. Every week must have: theme, goal, 2-4 specific topics, a GitHub-pushable deliverable.
9. Deliverable must be a working feature the student can see or test — never "read docs" or "research".
10. 2-4 milestones at meaningful turning points (e.g. first working UI, API connected, feature complete).
11. Output ONLY valid JSON, all strings single-line (no embedded newlines).
...
{{
  ...
  "weeks": [
    {{
      ...
      "end_date": "YYYY-MM-DD",
      "theme": "<what gets built this week>",
      "goal": "<what the student can show or run by end of week>",
      "topics": ["<topic needed to build it>", "<topic needed to build it>"],
      "deliverable": "<specific working feature pushed to GitHub>",
      "focus_areas": ["<any gap or focus area woven in, or empty list>"],
      "difficulty": "easy"
    }}
  ],
  "milestones": [
    {{"week": <int>, "description": "<what the project looks like at this point>"}}
  ]
}}"""
