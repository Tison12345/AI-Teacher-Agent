...
function WeeklyPhase({ sessionId, state, roadmap, onNextWeek, onComplete }) {
  ...
  async function skip() {
    setLoading(true); setError('')
    try { const res = await api.skipCompletion(sessionId); setCheckResult(res) }
    catch (e) { setError(e.message) }
    setLoading(false)
  }
  async function goToDashboard() {
    setLoading(true); setError('')
    try {
      const res = await api.skipToDashboard(sessionId)
        ...
        <>
            ...
            ? <div style={mk.card()}>
                <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexWrap: 'wrap', gap: 12 }}>
                  ...
                  <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap' }}>
                    <Btn variant="outline" onClick={check} disabled={loading} small>
                      {loading ? <><Spinner size={14} /> Checking…</> : 'Check GitHub →'}
                    </Btn>
                    <Btn variant="outline" onClick={skip} disabled={loading} small>
                      Skip
                    </Btn>
                    <Btn variant="green" onClick={goToDashboard} disabled={loading} small>
                      {loading ? <><Spinner size={14} /></> : 'Dashboard →'}
                    </Btn>
                  </div>
                </div>
              </div>
            : <div style={mk.card({ borderLeft: `3px solid ${checkResult.completed ? 'var(--accent3)' : 'var(--danger)'}` })}>
                ...
                {!checkResult.completed && (
                  <div style={{ display: 'flex', gap: 8, marginTop: 12 }}>
                    <Btn variant="outline" small onClick={check}>Re-check →</Btn>
                    <Btn variant="outline" small onClick={skip} disabled={loading}>Skip</Btn>
                    <Btn variant="green" small onClick={goToDashboard} disabled={loading}>Dashboard →</Btn>
                  </div>
                )}
              </div>
          }
          ...
          {checkResult?.completed && !quiz && !evaluation && (
            <div style={{ display: 'flex', gap: 8, justifyContent: 'flex-end', marginTop: 16 }}>
              <Btn variant="outline" onClick={goToDashboard} disabled={loading} small>
                {loading ? <><Spinner size={14} /></> : 'Dashboard →'}
              </Btn>
              <Btn onClick={loadQuiz} disabled={loading}>
                {loading ? <><Spinner size={16} /></> : 'Take the quiz →'}
              </Btn>
            </div>
          )}
